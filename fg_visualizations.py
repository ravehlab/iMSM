...
def create_wedge_marker(theta1_deg: float, theta2_deg: float) -> mpath.Path:
    ...
    n_pts: int = max(int(np.abs(theta2_deg - theta1_deg) / 4.0), 12)
    ...
    verts: list[tuple[float, float]] = (
        [(0.0, 0.0)]
        + [(float(xi), float(yi)) for xi, yi in zip(x, y)]
        + [(0.0, 0.0)]
    )
    codes: list[int] = (
        [mpath.Path.MOVETO]
...
def compute_heat_repeats_alignment_matrix(
    ...
    """Compute rotation matrix and Euler angles (X, Y, Z in deg) aligning HEAT repeats 5 & 6 plane to camera view with repeats parallel to Y axis."""
...
def visualize_kap_states_and_rates(
    ...
    if zoom:
        ...
        x_lim_min = center_x - half_span
        x_lim_max = center_x + half_span
        y_lim_min = center_y - half_span
        y_lim_max = center_y + half_span
        ...
        # Position Free State at top right corner of spatial bounding box if present
        if free_state_idx is not None:
            x_max: float = float(np.max(kap_2d[:, 0]))
            y_max: float = float(np.max(kap_2d[:, 1]))
            state_2d[free_state_idx] = np.array([x_max * 0.9, y_max * 1.1])
    ...
    ax.plot(
        ...
        linewidth=highlight_ribbon_w + 2.0,
        solid_capstyle="round",
        solid_joinstyle="round",
        zorder=3,
    )
    ...
    ax.plot(
        ...
        seg1_coords[:, 1],
        color="#FFAAAA",
        alpha=highlight_shine_alpha,
        linewidth=highlight_ribbon_w * 0.35,
        solid_capstyle="round",
        solid_joinstyle="round",
        zorder=3,
    )
    ...
    for i in range(n_clusters):
        ...
        # 2. Interacting pie slice (wedge without radial borders)
    ...
    sm = plt.cm.ScalarMappable(cmap=rate_cmap, norm=plt.Normalize(vmin=0.0, vmax=max_rate))
    sm.set_array([])
    cbar = fig.colorbar(sm, ax=ax, shrink=0.75, pad=0.02)
    cbar.set_label("Transition Rate (µs⁻¹)", fontsize=17.0)
    ...
    ax.set_xlabel("Spatial X Position (nm)", fontsize=17.0)
    ax.set_ylabel("Spatial Y Position (nm)", fontsize=17.0)
    ax.tick_params(labelsize=15.5)

    legend_handles, legend_labels = ax.get_legend_handles_labels()
    custom_proxies: list[object] = [
        Line2D(
            [0],
            [0],
            marker="o",
...
def find_state_exemplar_frames(
    ...
    with open(f"{imsm_checkpoint_path}/2_embedding.pickle", "rb") as f_embed:
        imsm_embed = pickle.load(f_embed)

    traj_labels: np.ndarray = imsm_cluster.trajectories[0].trajectory[0]
    embedded_sections: np.ndarray = imsm_embed.trajectories[0].trajectory[0]
    n_clusters: int = cluster_centers.shape[0]

    offset: int = first_frame if first_frame is not None else 0
    state_exemplars: list[dict[str, int | float]] = []

    for k in range(n_clusters):
        matching_indices: np.ndarray = np.where(traj_labels == k)[0]
        if len(matching_indices) > 0:
            sec_vectors: np.ndarray = embedded_sections[:, matching_indices].T
            dists: np.ndarray = np.linalg.norm(sec_vectors - cluster_centers[k], axis=1)
            best_sec_idx: int = int(matching_indices[np.argmin(dists)])
            min_dist: float = float(np.min(dists))
        else:
            all_sec_vectors: np.ndarray = embedded_sections.T
            dists = np.linalg.norm(all_sec_vectors - cluster_centers[k], axis=1)
            best_sec_idx = int(np.argmin(dists))
            min_dist = float(np.min(dists))

        start_frame: int = offset + (best_sec_idx * window_size) * stride
        end_frame: int = offset + ((best_sec_idx + 1) * window_size - 1) * stride
        mid_frame: int = offset + (best_sec_idx * window_size + window_size // 2) * stride
        sim_time_ns: float = mid_frame * ns_per_frame
        sim_time_us: float = sim_time_ns / 1000.0

        state_exemplars.append(
            {
                "state_id": k,
                "section_idx": best_sec_idx,
...
def generate_vmd_state_scripts(
    ...
    focal_res_min: int = max(862, min(focal_res_seqs) - 5)
    focal_res_max: int = min(986, max(focal_res_seqs) + 5)
    ...
    bound_mask: np.ndarray = (
        ...
        if free_state_idx is None
        else np.arange(cluster_centers.shape[0]) != free_state_idx
    )
    bound_states_3d_angstrom: np.ndarray = state_xyz_angstrom[bound_mask]
